...
//    for(i=0;i<MAX_EDGE_NUM;i++) nodeAngle[i]=-1.0e+32;
    ...
    edges.resize(MAX_EDGE_NUM, nullptr);
    edgeDir.resize(MAX_EDGE_NUM, false);
    nodeAngle.resize(MAX_EDGE_NUM, -1.0e+32);
}
...
{
    if (whichEdge >= edgeDir.size()) edgeDir.resize(whichEdge + 1);
...
{
    if (whichEdge >= edges.size()) edges.resize(whichEdge + 1);
    edges[whichEdge]=_edge;
    edgeNum = std::max(edgeNum, whichEdge);
}
...
{
    if (whichNode >= nodeAngle.size()) nodeAngle.resize(whichNode + 1);
